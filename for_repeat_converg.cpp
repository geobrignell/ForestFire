...
std::tuple<int, int> forest_fire(int N, double p);
...
int main(int argc, char **argv)
{
  ...
  assert (argc == 4); // removed seed so check only 2 args (which is 3 inlucing name)
  int N = atoi(argv[1]);
  double p = atof(argv[2]);
  int tot_seed = atoi(argv[3]);
  ...
  std::string folder_name = "data/data_with_s_" + std::to_string(tot_seed) + "/s_" + std::to_string(tot_seed);
  ...
  for (int seed = 0; seed < tot_seed; seed++){
    myfile << seed << ",";
    srand(seed);
    myfile << std::get<0>(forest_fire(N, p)) << "," << std::get<1>(forest_fire(N, p)) << "\n";
  }
